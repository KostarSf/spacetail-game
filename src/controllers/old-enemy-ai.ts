...
export class OldEnemyAI extends TickableController {
    ...
    onTakeDamage(_ship: Ship, _amount: number, _angle: number): void {
        _ship.vel = _ship.vel.rotate(degToRad(random.integer(5, 25) * random.pickOne([1, -1])));
    ...
    onTick(_engine: Engine, _ship: Ship): void {
        ...
        const maxDistance = this.#targetSpotted ? this.#purcuitDistance : this.#spottedDistance;
